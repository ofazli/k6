import http from "k6/http";
import { sleep } from "k6";
import { Counter, Trend } from "k6/metrics";

export const options = {
  vus: 5,
  duration: "5s",
  thresholds: {
    http_req_duration: ["p(95)<200", "max<200"],
    my_counter: ["count>10"],
    response_time_news_page: ["p(95)<50", "p(99)<50"],
  },
};

let myCounter = new Counter("my_counter");
let newsPageResponseTrend = new Trend("response_time_news_page");

export default () => {
  let res = http.get("https://quickpizza.grafana.com/test.k6.io/");
  myCounter.add(1);
  sleep(1);

  res = http.get("https://quickpizza.grafana.com/news.php");
  newsPageResponseTrend.add(res.timings.duration);
  sleep(1);
};

/*
Custom metrics are metrics that are not part of the default metrics. You can create your own metrics and use them in your tests.
*/
